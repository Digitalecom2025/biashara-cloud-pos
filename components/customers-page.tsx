  ...
  return (
    <div className="mx-auto max-w-[1700px]">
      <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end">
        ...
        <button className="flex w-fit items-center gap-2 rounded-xl bg-[#16A34A] px-4 py-3 text-xs font-black text-white shadow-lg shadow-[#16A34A]/15 hover:bg-[#12883E]">
        ...
        <article className="overflow-hidden rounded-2xl border border-[#DDEAE0] bg-white shadow-sm shadow-[#12311F]/5">
          <div className="flex flex-col gap-3 border-b border-[#E8F0EA] p-4 sm:flex-row">
            <label className="relative flex-1">
              ...
              <input value={query} onChange={(event) => setQuery(event.target.value)} aria-label="Search customers" placeholder="Search customer, phone or email..." className="w-full rounded-xl border border-[#DDEAE0] bg-[#F8FBF8] py-2.5 pl-9 pr-3 text-xs outline-none placeholder:text-[#9AAEA3] focus:border-[#16A34A]" />
            </label>
            <label className="relative">
              <select value={status} onChange={(event) => setStatus(event.target.value)} aria-label="Filter customers by status" className="w-full appearance-none rounded-xl border border-[#DDEAE0] bg-white py-2.5 pl-3 pr-9 text-xs font-bold text-[#60766B] outline-none focus:border-[#16A34A] sm:min-w-40">
                <option>All statuses</option><option>Clear</option><option>Owes</option><option>Overdue</option>
              </select>
            ...
            <table className="w-full min-w-[1160px] border-collapse text-left">
              ...
              <tbody>{filteredCustomers.map((customer) => (
                <tr key={customer.id} onClick={() => setSelectedId(customer.id)} className={`cursor-pointer border-t border-[#EEF3EF] text-xs text-[#60766B] hover:bg-[#FBFDFB] ${selectedId === customer.id ? "bg-[#16A34A]/[0.035]" : ""}`}>
                  ...
                  <td className="px-4 py-3 text-right"><button aria-label={`Actions for ${customer.name}`} className="grid h-8 w-8 place-items-center rounded-lg text-[#789083] hover:bg-[#F5FAF6]"><MoreHorizontal size={16} /></button></td>
                </tr>
              ))}</tbody>
            </table>
          </div>
          ...
          <div className="grid gap-3 p-3 lg:hidden">
            {filteredCustomers.map((customer) => (
              <button key={customer.id} onClick={() => setSelectedId(customer.id)} className={`rounded-xl border p-3 text-left ${selectedId === customer.id ? "border-[#16A34A]/60 bg-[#16A34A]/[0.035]" : "border-[#E8F0EA]"}`}>
...
function StatusBadge({ status }: { status: CustomerStatus }) {
  const tone = status === "Clear" ? "bg-[#16A34A]/10 text-[#0F8C42]" : status === "Owes" ? "bg-[#D4A017]/12 text-[#9A7108]" : "bg-[#EF4444]/10 text-[#EF4444]";
...
function CustomerProfile({ customer }: { customer: Customer }) {
  return (
    <aside className="overflow-hidden rounded-2xl border border-[#DDEAE0] bg-white shadow-sm shadow-[#12311F]/5 xl:sticky xl:top-[96px]">
      ...
      <div className="space-y-2 p-4"><button className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#16A34A] py-3 text-xs font-black text-white hover:bg-[#12883E]">View full profile <ArrowUpRight size={14} /></button><button className="w-full rounded-xl border border-[#DDEAE0] py-3 text-xs font-black text-[#60766B]">Customer statement</button></div>
    </aside>
  );
}
