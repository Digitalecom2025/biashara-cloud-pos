...
function SupplierProfile({ supplier, onFeedback }: { supplier: Supplier; onFeedback: (message: string) => void }) {
  return <aside className="overflow-hidden rounded-2xl border border-[#DDEAE0] bg-white shadow-sm shadow-[#12311F]/5 xl:sticky xl:top-[96px]"><div className="bg-[#12311F] p-5 text-white"><div className="flex items-start justify-between"><span className="grid h-14 w-14 place-items-center rounded-2xl bg-white/10 text-sm font-black text-[#22C55E]">{supplier.initials}</span><Truck size={19} className="text-[#B8C7BD]" /></div><h3 className="mt-4 text-lg font-black">{supplier.name}</h3><p className="mt-1 text-[11px] text-[#B8C7BD]">{supplier.category}</p></div><div className="space-y-3 p-4 text-xs"><p className="flex items-center gap-2 text-[#60766B]"><Phone size={14} className="text-[#16A34A]" /> {supplier.phone}</p><p className="flex items-center gap-2 text-[#60766B]"><Mail size={14} className="text-[#16A34A]" /> {supplier.email || "No email added"}</p><p className="flex items-center gap-2 text-[#60766B]"><Building2 size={14} className="text-[#16A34A]" /> {supplier.status} account</p></div><div className="grid grid-cols-2 gap-px bg-[#E8F0EA]"><ProfileStat icon={ShoppingBag} label="Purchases" value={formatCurrency(supplier.totalPurchases)} /><ProfileStat icon={WalletCards} label="Balance" value={formatCurrency(supplier.currentBalance)} danger={supplier.currentBalance > 0} /></div><div className="space-y-2 p-4"><button disabled title="Detailed supplier profile coming soon" className="flex w-full cursor-not-allowed items-center justify-center gap-2 rounded-xl bg-[#CBD8CF] py-3 text-xs font-black text-white">Profile coming soon <ArrowUpRight size={14} /></button><button disabled title="Supplier PDF statements coming soon" className="w-full cursor-not-allowed rounded-xl border border-[#DDEAE0] bg-[#F5FAF6] py-3 text-xs font-black text-[#9AAEA3]">Statement coming soon</button></div></aside>;
}
