...
function BranchProfile({ branch }: { branch: Branch; onFeedback: (message: string) => void }) { return <aside className="overflow-hidden rounded-2xl border border-[#DDEAE0] bg-white xl:sticky xl:top-[96px]"><div className="bg-[#12311F] p-5 text-white"><span className="grid h-14 w-14 place-items-center rounded-2xl bg-white/10 text-sm font-black text-[#22C55E]">{branch.initials}</span><h3 className="mt-4 text-lg font-black">{branch.name}</h3><p className="mt-1 text-[11px] text-[#B8C7BD]">{branch.focus}</p></div><div className="space-y-3 p-4 text-xs text-[#60766B]"><p className="flex gap-2"><MapPin size={14} className="text-[#16A34A]" />{branch.location}</p><p className="flex gap-2"><Phone size={14} className="text-[#16A34A]" />{branch.phone || "No phone added"}</p><p className="flex gap-2"><Users size={14} className="text-[#16A34A]" />Manager: {branch.manager}</p></div><div className="grid grid-cols-2 gap-px bg-[#E8F0EA]"><ProfileStat label="Stock value" value={formatCurrency(branch.stockValue)} icon={WalletCards} /><ProfileStat label="Sales today" value={formatCurrency(branch.salesToday)} icon={CircleDollarSign} /></div><div className="space-y-2 p-4"><Link href={`/branches/${branch.id}/dashboard`} className="block w-full rounded-xl bg-[#16A34A] py-3 text-center text-xs font-black text-white hover:bg-[#12883E]">View branch dashboard</Link><Link href={`/branches/${branch.id}`} className="block w-full rounded-xl border border-[#DDEAE0] bg-[#F5FAF6] py-3 text-center text-xs font-black text-[#60766B] hover:bg-[#EEF7F0]">View branch profile</Link></div></aside>; }
