...
export function AiAssistantPage() {
  ...
  const [response, setResponse] = useState(mockAiResponses[suggestedPrompts[7]]);
  ...
  function askAi(prompt: string) {
    ...
    setResponse(mockAiResponses[cleanPrompt] ?? fallbackAiResponse);
  }
  ...
  return (
    <div className="mx-auto max-w-[1800px]">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-[#E9D7A0]">
                  AI Business Assistant will be available on Premium and Custom plans. Lite and Growth users can upgrade to unlock AI insights.
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
        {aiSummaryCards.map((card) => <SummaryCard key={card.title} card={card} />)}
      </section>
      ...
      <section className="mt-5 grid gap-5 xl:grid-cols-2">
        <InsightBlock icon={TrendingUp} title="Sales Insights" items={salesInsights} />
        <InsightBlock icon={PackageSearch} title="Stock Insights" items={stockInsights} />
        <InsightBlock icon={ShieldAlert} title="Debtor Insights" items={debtorInsights} />
        <InsightBlock icon={WalletCards} title="Expense Insights" items={expenseInsights} />
        <InsightBlock icon={Users} title="Staff / Cashier Insights" items={staffInsights} wide />
      </section>
...
function SummaryCard({ card }: { card: AiSummaryCard }) {
      ...
      <div className="border-b border-[#E8F0EA] bg-[#F8FBF8] p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#789083]">Mock chat panel. No real AI API is connected.</p>
          </div>
        </div>
      </div>
    ...
    <article className="rounded-2xl border border-[#D4A017]/35 bg-[#FFF9E8] p-5 shadow-sm shadow-[#12311F]/5">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-[#8A670C]">Mock AI guidance based on the current demo data.</p>
        </div>
      </div>
...
function InsightBlock({ icon: Icon, title, items, wide }: { icon: typeof TrendingUp; title: string; items: AiInsightItem[]; wide?: boolean }) {
  return (
    <article className={`rounded-2xl border border-[#DDEAE0] bg-white p-5 shadow-sm shadow-[#12311F]/5 ${wide ? "xl:col-span-2" : ""}`}>
      <div className="flex items-center gap-3">
        <span className="grid h-10 w-10 place-items-center rounded-xl bg-[#16A34A]/10 text-[#16A34A]">
          <Icon size={18} />
        </span>
        <div>
          ...
          <p className="text-xs text-[#789083]">Mock insight summary for this business area.</p>
        </div>
      </div>
